...
- 네이버 상품명은 35~45자 전후를 우선하고 최대 50자를 넘기지 않는다. 너무 짧게 압축하지 말고 `상품 정체성 + 핵심 기능 + 용도/사용처 + 대표 속성`까지 담는다.
...
- 쿠팡은 40~55자 전후로 가장 풍부하게 만든다. 롯데ON/11번가/ESM은 35~50자 전후로 만들되, 마켓별 성격에 맞춰 길이를 조절한다.
...
- 같은 예시는 `앞머리 뿌리 볼륨 집게 헤어 그루프 고정 스타일링 셀프 미용소품`처럼 반복 축을 제거한 뒤, 남는 길이는 `용도/대상/문제상황/형태/사용상황`으로 보강한다. `뿌리볼륨집게`, `앞머리볼륨`, `헤어볼륨집게`처럼 이미 나온 축을 붙여쓴 말로 길이를 채우지 않는다.
...
- 마켓별 생성 방식 (5개 마켓은 서로 다른 방식을 쓴다. 같은 키워드 묶음을 복사하면 안 된다):
  - 네이버 = [하이브리드 SEO형]: ①집중형처럼 너무 짧게 만들지 않는다. 핵심 검색어를 앞에 두되 `대표 품목명 + 핵심 기능 + 용도/사용처 + 대표 속성`을 중상 길이로 구성한다. tags 8~10개.
  - 쿠팡 = [롱테일 확장형]: 규격·호환·기능·용도·구성·문제해결 조합을 가장 넓게 만든다. 상품명도 가장 길게 쓰고 키워드도 가장 많게 만든다(18~25개).
  - ESM = [용도·문제해결형]: 교체/누유/수리/정비/보강/고정/정리처럼 구매의도·문제해결·사용상황 중심. 표준명+다른명칭+행사/시즌(14~20개).
  - 11번가 = [하이브리드 균형형]: 정체성+규격+용도를 고르게 배치. 동의어·현장명 보강(14~20개).
  - 롯데ON = [실사용 중간형]: ⑤압축형처럼 짧게 만들지 않는다. 상품명은 최소 7~9개 단어 이상으로 `표준명 + 핵심 기능 + 사용처 + 소재/규격 + 실사용 맥락`을 구성한다. 생활/공간/사용처 맥락을 2~4개 보강한다(12~18개).

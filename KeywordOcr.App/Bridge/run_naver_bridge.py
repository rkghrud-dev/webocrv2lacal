"""
C# → Python 네이버 스마트스토어 업로드 bridge
...
def main() -> int:
    parser = argparse.ArgumentParser(description="네이버 업로드 bridge")
    ...
    _status("네이버 스마트스토어 업로드 bridge 시작")

    from app.services.naver_commerce import NaverUploadConfig, run_naver_upload

    dry_run = args.dry_run.lower() in ("true", "1", "yes")

    cfg = NaverUploadConfig(
    ...
    results = run_naver_upload(cfg, status_cb=_status, progress_cb=None)
    ...
    for r in results:
        ...
        if r.status in ("OK", "DRY_RUN_OK"):
            success_count += 1
        else:
            fail_count += 1
            _status(f"  [실패] 행{r.row} {r.name[:30]} → {r.status}: {r.error[:200]}")
